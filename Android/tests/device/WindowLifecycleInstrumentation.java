...
/** Regression for copying pairing information via another app, then connecting from Mac. */
public final class WindowLifecycleInstrumentation extends SyncInstrumentation {
    ...
    void awaitPair()throws Exception{long end=System.currentTimeMillis()+10000;while(SyncWindowService.pairing().isEmpty()&&System.currentTimeMillis()<end)Thread.sleep(25);check(!SyncWindowService.pairing().isEmpty(),"Unpaired manual window did not show pairing information");}
    ...
    @Override void runTests()throws Exception{
        ...
        MainActivity first=foreground();runOnMainSync(()->first.syncServer.start(false,true));awaitPair();
        String[] pairing=SyncWindowService.pairing().split("\\.");pin=pairing[1];runOnMainSync(()->first.moveTaskToBack(true));Thread.sleep(3000);
        check(SyncWindowService.running(),"switching apps closed the pairing window");
        token=post("pair",new JSONObject().put("code",pairing[0]),"").getString("token");check(!token.isEmpty(),"pairing failed while Activity stopped");
